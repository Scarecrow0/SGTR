...
def get_dataset_statistics(cfg):
    ...
    data_statistics_name = ''.join(dataset_names.split('_')[0]) + '_statistics'
    save_file = os.path.join(cfg.OUTPUT_DIR, "{}.cache".format(data_statistics_name))
    print('Loading data statistics from: ' + str(save_file))
    stat = torch.load(save_file, map_location=torch.device("cpu"))
    return stat
